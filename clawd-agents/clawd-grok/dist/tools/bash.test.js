...
import { BashTool, getSandboxMutationBlockReason, shouldRunOnHostInSandboxMode, wrapCommandForShuru, wrapHostBrowserCommand, } from "./bash";
const tempDirs = [];
...
describe("shouldRunOnHostInSandboxMode", () => {
    ...
    it("allows compound commands with only safe prefixes like mkdir and sleep", () => {
        expect(shouldRunOnHostInSandboxMode("mkdir -p .grok/verify-artifacts && agent-browser --session verify open http://127.0.0.1:3000", { hostBrowserCommandsOnHost: true })).toBe(true);
        expect(shouldRunOnHostInSandboxMode("sleep 5 && agent-browser screenshot out.png", { hostBrowserCommandsOnHost: true })).toBe(true);
        expect(shouldRunOnHostInSandboxMode("mkdir -p .grok/verify-artifacts && agent-browser open http://127.0.0.1:3000 && agent-browser screenshot", { hostBrowserCommandsOnHost: true })).toBe(true);
    });
...
describe("wrapHostBrowserCommand", () => {
    ...
    it("handles session flags and screenshot paths correctly", () => {
        const result = wrapHostBrowserCommand("agent-browser --session verify screenshot .grok/verify-artifacts/verify-smoke-home.png");
