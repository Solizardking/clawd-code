...
function getSchedulesDirPath() {
    return path.join(getScheduleHomeBase(), ".grok", "schedules");
}
function getScheduleDaemonPidFilePath() {
    return path.join(getScheduleHomeBase(), ".grok", "daemon.pid");
}
let scheduleHomeBaseOverride = null;
function getScheduleHomeBase() {
    return scheduleHomeBaseOverride ?? os.homedir();
}
export class ScheduleManager {
    getCwd;
    getModel;
    constructor(getCwd = () => process.cwd(), getModel = () => getCurrentModel()) {
        this.getCwd = getCwd;
    ...
    async touchLastRunAt(id, at = new Date()) {
        const schedule = await this.require(id);
...
export async function ensureSchedulesDir() {
...
export function getScheduleRunLogPath(id) {
    return path.join(getScheduleLogDir(id), "run.log");
}
export function getScheduleDaemonPidPath() {
    return getScheduleDaemonPidFilePath();
}
export async function writeScheduleDaemonPid(pid) {
    const pidPath = getScheduleDaemonPidFilePath();
    await fs.mkdir(path.dirname(pidPath), { recursive: true });
    await fs.writeFile(pidPath, `${pid}\n`, "utf8");
}
export async function removeScheduleDaemonPid() {
    await fs.rm(getScheduleDaemonPidFilePath(), { force: true });
}
export async function getScheduleDaemonStatus() {
    try {
...
export async function startScheduleDaemon(cwd = process.cwd()) {
...
export async function stopScheduleDaemon() {
    const existing = await getScheduleDaemonStatus();
...
export async function listStoredSchedules() {
    return new ScheduleManager().list();
}
export async function readStoredSchedule(id) {
    return readScheduleRecord(getScheduleRecordPath(id));
}
export async function writeStoredSchedule(schedule) {
    await writeScheduleRecord(schedule);
}
export async function startDetachedHeadlessRun(options) {
...
export function buildHeadlessCliArgs(options) {
    return [
        "--directory",
        options.directory,
        "--prompt",
...
async function waitForDaemonStatus(pid, shouldBeRunning, attempts = 20, delayMs = 50) {
...
export function toScheduleId(name) {
    const id = name
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9-]+/g, "-")
...
function parseCronExpression(expr) {
    ...
    if (!validateCronField(month, 1, 12))
        return null;
    if (!validateCronField(dayOfWeek, 0, 7, true))
        return null;
    return { minute, hour, dayOfMonth, month, dayOfWeek };
}
function validateCronField(field, min, max, dayOfWeek = false) {
    const parts = field.split(",");
    if (parts.length === 0)
        return false;
    return parts.every((part) => parseCronPart(part.trim(), min, max, dayOfWeek) !== null);
}
function matchesCronField(field, value, min, max, dayOfWeek = false) {
    ...
    return field.split(",").some((part) => {
        ...
        if (parsed.start <= parsed.end) {
            if (normalizedValue < parsed.start || normalizedValue > parsed.end)
                return false;
...
function parseCronPart(part, min, max, dayOfWeek) {
    if (!part)
        return null;
    ...
    if (base.includes("-")) {
        ...
        if (dayOfWeek) {
            ...
            if (startRaw < min || startRaw > max || endRaw < min || endRaw > max)
                return null;
            if (startRaw > endRaw)
                return null;
        }
...
function parseCronNumber(raw, min, max, dayOfWeek) {
...
function normalizeCronValue(value, dayOfWeek) {
    if (dayOfWeek && value === 7)
        return 0;
    return value;
}
async function resolveScheduleDirectory(directory, cwd) {
...
async function readScheduleRecord(filePath) {
    try {
...
async function writeScheduleRecord(schedule) {
    await ensureSchedulesDir();
