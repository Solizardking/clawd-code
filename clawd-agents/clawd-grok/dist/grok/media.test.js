...
describe("media tools", () => {
    ...
    it("generates edited images from a local source path and saves them under .grok/generated-media", async () => {
        ...
        const provider = {
            image: vi.fn((modelId) => ({ modelId })),
        };
        const result = await generateImageTool(provider, {
    ...
    it("generates videos from a remote source image and respects output path and polling options", async () => {
        globalThis.fetch = vi.fn().mockResolvedValue(new Response(Buffer.from([5, 4, 3, 2]), {
            status: 200,
        ...
        const provider = {
            video: vi.fn((modelId) => ({ modelId })),
        };
        const result = await generateVideoTool(provider, {
        ...
        const prompt = generateVideoMock.mock.calls[0]?.[0]?.prompt;
