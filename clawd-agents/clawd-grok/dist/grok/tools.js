...
import { computerClick, computerFocusWindow, computerGet, computerLaunch, computerListWindows, computerMouseMove, computerPress, computerScreenshot, computerScroll, computerSnapshot, computerType, computerWait, } from "../tools/computer";
...
import { loadPaymentSettings, loadValidSubAgents } from "../utils/settings";
import { generateImageTool, generateVideoTool, IMAGE_ASPECT_RATIOS, IMAGE_RESOLUTIONS, VIDEO_ASPECT_RATIOS, VIDEO_RESOLUTIONS, } from "./media";
...
export function createTools(bash, provider, mode = "agent", options = {}) {
    ...
    const runResponsesSearch = async (query, toolName, abortSignal) => {
    ...
    const base = {
        bash: tool({
            ...
            execute: async ({ command, timeout, background }, { abortSignal }) => {
                ...
                if (result.success) {
                    ...
                    executePostToolFailureHooks("bash", toolInput, result.error || "Command failed", cwd(), options.sessionId, abortSignal).catch(() => { });
                }
                return output;
            },
        }),
        ...
        generate_image: tool({
            description: "Generate a new image or edit an existing image using Grok Imagine. Use when the user asks to create, redesign, restyle, or modify an image. Optionally pass a local file path or public URL in source to edit an existing image. Saves the generated image files locally and returns their paths.",
            inputSchema: z.object({
    ...
    if (enabledToolsets.has("camsnap")) {
        tools.camsnap_discover = tool({
            description: "Discover configured or visible cameras using camsnap. Use this before snapping or recording when the camera name is unknown.",
            inputSchema: z.object({
        ...
        tools.camsnap_doctor = tool({
            description: "Run camsnap diagnostics. Use when camera capture fails, ffmpeg is missing, or the camera config may be invalid.",
            inputSchema: z.object({
        ...
        tools.camsnap_snap = tool({
            description: "Capture one still image from a configured camera with camsnap. Prefer a short test capture before asking for longer clips.",
            inputSchema: z.object({
        ...
        tools.camsnap_clip = tool({
            description: "Record a short video clip from a configured camera with camsnap. Keep duration short unless the user explicitly asks for longer.",
            inputSchema: z.object({
                camera: z.string().describe("Camera name or id from camsnap config/discover"),
        ...
        tools.camsnap_watch = tool({
            description: "Run a bounded camsnap motion watch for one camera. Use only when the user explicitly asks to monitor for motion.",
            inputSchema: z.object({
                camera: z.string().describe("Camera name or id from camsnap config/discover"),
    ...
    if (isLspToolEnabled(cwd())) {
        tools.lsp = tool({
            description: "Experimental Language Server Protocol access for semantic code intelligence. Use for go-to-definition, references, hover, symbols, implementations, and call hierarchy when a matching LSP server is available.",
            inputSchema: z.object({
                ...
                line: z
                    ...
                    .min(1)
                    .optional()
                    .describe("The 1-based line number. Required for position-based operations (definition, references, hover, implementation, callHierarchy). Not needed for documentSymbol or workspaceSymbol."),
                character: z
                    .number()
                    .int()
                    .min(1)
                    .optional()
                    .describe("The 1-based character offset. Required for position-based operations. Not needed for documentSymbol or workspaceSymbol."),
    ...
    if (options.sendTelegramFile) {
        ...
        tools.telegram_send_file = tool({
            description: "Send a local file to the current Telegram chat as an attachment. Use this to deliver generated images, videos, documents, or any other file to the user in Telegram. The file is uploaded directly — the user receives it as a Telegram media message or document.",
            inputSchema: z.object({
    ...
    if (options.runTask) {
        ...
        const customHint = customNames.length > 0
    ...
    if (mode === "agent") {
        tools.computer_snapshot = tool({
            description: "Capture a semantic accessibility snapshot of a desktop app using agent-desktop. Prefer this before desktop interaction. It returns stable refs like @e1 that remain valid until the next snapshot.",
            inputSchema: z.object({
        ...
        tools.computer_screenshot = tool({
            description: "Capture a PNG screenshot of a desktop app window using agent-desktop. Use for visual confirmation or when the accessibility snapshot is insufficient.",
            inputSchema: z.object({
                output_path: z
                    .string()
                    .optional()
        ...
        tools.computer_click = tool({
            description: "Click a desktop element via agent-desktop. Prefer `ref` values from computer_snapshot. Coordinates are a fallback only when accessibility refs are unavailable.",
            inputSchema: z.object({
        ...
        tools.computer_type = tool({
            description: "Type text into a specific desktop element via agent-desktop. Pass a ref from computer_snapshot. For shortcuts like cmd+k or enter, prefer computer_press.",
            inputSchema: z.object({
        ...
        tools.computer_list_windows = tool({
            description: "List visible desktop windows using agent-desktop. Use this to discover window ids before focusing or scoping snapshots.",
            inputSchema: z.object({
        ...
        tools.computer_wait = tool({
            description: "Wait for time, an element ref, a window title, or text to appear using agent-desktop. Use after launches, dialogs, or UI transitions.",
            inputSchema: z.object({
                milliseconds: z
                    .number()
                    .int()
        ...
        if (options.scheduleManager) {
            ...
            tools.schedule_create = tool({
                description: "Create a recurring or one-time scheduled headless Grok run. Provide a name, the instruction to run, and a cron expression for recurring schedules. Omit cron for an immediate one-time run.",
                inputSchema: z.object({
            ...
            tools.schedule_list = tool({
                description: "List all saved schedules, including one-time and recurring runs, along with daemon status and last run time.",
                inputSchema: z.object({}),
                execute: async () => {
                    try {
            ...
            tools.schedule_daemon_start = tool({
                description: "Start the schedule daemon in the background so recurring schedules can run even after the TUI closes.",
                inputSchema: z.object({}),
                execute: async () => {
                    try {
    ...
    tools.wallet_info = tool({
        description: "Get the local wallet address, chain, and current balances (ETH and USDC). Use this to check available funds before making a payment.",
        inputSchema: z.object({}),
        execute: async () => {
            try {
    ...
    tools.wallet_history = tool({
        description: "Show recent x402 payment history from the local audit log. Returns the most recent payment attempts with status, URL, amount, and transaction hash.",
        inputSchema: z.object({
    ...
    tools.fetch_payment_info = tool({
        description: "Inspect a URL for x402 payment requirements without paying. Returns payment options and a brin security scan with score, sub-scores (identity, behavior, content, graph), and any detected threats. Use this only when the user wants to inspect without paying — for actual access, call paid_request directly instead.",
        inputSchema: z.object({
    ...
    tools.paid_request = tool({
        description: "Access an x402-protected URL using the local wallet. URLs scoring below 25 on brin are automatically blocked. The user will be prompted to approve the payment before it executes.",
        inputSchema: z.object({
...
function formatScheduleList(schedules, daemonStatus) {
    const lines = [
...
function formatDaemonReminder(status) {
    if (status.running) {
