...
function isStrictDescendantOf(ancestor, node) {
    let p = node.parent;
    while (p) {
        if (p === ancestor)
            return true;
...
function selectedTextBuffersOnly(selection) {
    const raw = selection.selectedRenderables.filter((r) => !r.isDestroyed && r instanceof TextBufferRenderable);
 ...
 */
export function getCompactTuiSelectionText(selection) {
...
//# sourceMappingURL=terminal-selection-text.js.map