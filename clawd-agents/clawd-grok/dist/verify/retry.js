...
function buildNativeModuleRetry(profile) {
    if (!NODE_WEB_APP_KINDS.has(profile.appKind)) {
        return null;
    }
    const installCommand = profile.packageManager === "bun"
...
function buildStartupRetry(profile) {
...
export function getVerifyRetryStrategies(profile) {
    return [buildNativeModuleRetry(profile), buildStartupRetry(profile)].filter((strategy) => Boolean(strategy));
}
export function buildRetryGuidance(profile) {
