...
import { mergeSandboxSettings, normalizeSandboxSettings } from "../utils/settings";
...
function normalizeManifestRecipe(raw) {
...
export function loadVerifyEnvironment(cwd, baseSettings = {}) {
...
function pickPersistentSandboxSettings(settings) {
    const persistent = {};
    if (settings.allowNet !== undefined)
        persistent.allowNet = settings.allowNet;
    if (settings.allowedHosts?.length)
        persistent.allowedHosts = settings.allowedHosts;
    if (settings.ports?.length)
        persistent.ports = settings.ports;
    if (settings.cpus)
        persistent.cpus = settings.cpus;
    if (settings.memory)
        persistent.memory = settings.memory;
    if (settings.diskSize)
        persistent.diskSize = settings.diskSize;
    if (settings.secrets?.length)
        persistent.secrets = settings.secrets;
    if (settings.from)
        persistent.from = settings.from;
    if (settings.verifyBaseFrom)
        persistent.verifyBaseFrom = settings.verifyBaseFrom;
    if (settings.guestWorkdir)
        persistent.guestWorkdir = settings.guestWorkdir;
    if (settings.syncHostWorkspace !== undefined)
        persistent.syncHostWorkspace = settings.syncHostWorkspace;
    if (settings.shellInit?.length)
        persistent.shellInit = settings.shellInit;
    return persistent;
}
