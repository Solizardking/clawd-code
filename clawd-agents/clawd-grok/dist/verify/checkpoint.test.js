...
describe("verify checkpoints", () => {
    ...
    it("deletes a failed checkpoint when bootstrap/install fails", async () => {
        ...
        await expect(ensureVerifyCheckpoint(dir, profile, profile.sandboxSettings)).rejects.toThrow(`Verify checkpoint bootstrap failed for "${checkpointName}"`);
