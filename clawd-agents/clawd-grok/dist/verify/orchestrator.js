...
import { inferVerifyProjectProfile } from "./recipes";
function buildRuntimeSandboxSettings(profile) {
    return {
...
export async function prepareVerifyRun(agent, options = {}) {
    ...
    if (!manifest) {
        ...
        if (detectedRecipe) {
            ...
            options.onProgress?.("verify-detect did not return a usable recipe; keeping deterministic fallback without writing a manifest");
        }
    }
    ...
    if (checkpoint.checkpointName) {
        ...
        options.onProgress?.(checkpoint.created
...
export async function runVerifyOrchestration(agent, options = {}) {
