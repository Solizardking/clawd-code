...
async function importLoadCustomInstructions(mockedHome) {
    vi.resetModules();
    vi.doUnmock("os");
    if (mockedHome) {
...
describe("loadCustomInstructions", () => {
    ...
    it("loads global plus repo-chain AGENTS files in order", async () => {
        ...
        const loadCustomInstructions = await importLoadCustomInstructions(home);
        expect(loadCustomInstructions(cwd)).toBe(["global instructions", "root instructions", "pkg instructions", "feature instructions"].join("\n\n"));
    });
