...
function readStoredDelegationRecord(home) {
    ...
    const projectDir = path.join(delegationsRoot, projectDirs[0]);
    const files = fs.readdirSync(projectDir).filter((file) => file.endsWith(".json"));
    expect(files).toHaveLength(1);
    return JSON.parse(fs.readFileSync(path.join(projectDir, files[0]), "utf8"));
}
async function importDelegationsModule(options = {}) {
    vi.resetModules();
...
describe("DelegationManager sandbox propagation", () => {
    it("persists sandbox mode in background delegation records", async () => {
        ...
        expect(spawnMock).toHaveBeenCalledWith(process.execPath, expect.arrayContaining(["--directory", cwd]), expect.objectContaining({ cwd }));
        const record = readStoredDelegationRecord(home);
