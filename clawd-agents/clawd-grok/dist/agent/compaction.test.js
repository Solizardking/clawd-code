...
import { buildEffectiveTranscript } from "../storage/transcript-view";
import { COMPACTION_SUMMARY_HEADER, createCompactionSummaryMessage, findCutPoint, prepareCompaction, serializeConversation, shouldCompactContext, } from "./compaction";
function user(text) {
...
function toolResult(toolCallId, toolName, output) {
    return {
        role: "tool",
...
describe("compaction helpers", () => {
    ...
    it("builds the effective transcript from the latest persisted checkpoint", () => {
        ...
        expect(transcript.messages[0]?.role === "system" && typeof transcript.messages[0].content === "string"
