...
function findCustomSubagent(agent, subagents = loadValidSubAgents()) {
    return (subagents.find((item) => item.name === agent) ??
        subagents.find((item) => item.name.toLowerCase() === agent.toLowerCase()));
}
function formatCustomSubagentsPromptSection(subagents) {
...
export class Agent {
    ...
    async refreshSessionRecap(signal) {
    ...
    async runTaskRequest(request, onActivity, abortSignal) {
        ...
        const custom = !isExplore && !isGeneral && !isVision && !isVerify && !isVerifyDetect && !isVerifyManifest && !isComputer
        ...
        const verifySandboxOverrides = isVerify
            ...
            : {};
        let verifyPreparedSettings = null;
        let verifyPreparedRecipe = null;
        if (isVerify) {
            const prepared = await prepareVerifySandbox(this.bash.getCwd(), { ...this.bash.getSandboxSettings(), ...verifySandboxOverrides }, undefined, onActivity);
        ...
        if (isComputer && childRuntime.modelInfo?.supportsClientTools === false) {
            return {
                success: false,
                output: "Computer sub-agent requires a tool-capable model, but the selected runtime does not support client tools.",
                task: {
                    agent: agentKey,
                    description: request.description,
        ...
        try {
            ...
            const childPrompt = isVerify && verifyPreparedRecipe
    ...
    async runTask(request, abortSignal) {
        ...
        let result;
        try {
            result = await this.runTaskRequest(request, (detail) => {
    ...
    async *processMessage(userMessage, observer) {
        ...
        const userModelMessage = userModelMessages[0] ?? { role: "user", content: userMessage };
...
function formatSubagentActivity(toolName, args) {
    ...
    if (toolName === "lsp")
        return `LSP ${parsed.operation || "query"} ${parsed.filePath || ""}`.trim();
    ...
    if (toolName === "computer_snapshot")
        return `Snapshot ${parsed.app || "desktop"}`;
    ...
    if (toolName === "computer_type")
        return `Type into ${parsed.ref || "element"}`;
    if (toolName === "computer_press")
        return `Press ${parsed.key || "key"}`;
    if (toolName === "computer_scroll")
        return `Scroll ${parsed.ref || "element"} ${parsed.direction || "down"}`;
    if (toolName === "computer_launch")
        return `Launch ${parsed.app || "app"}`;
    if (toolName === "computer_list_windows")
        return `List windows${parsed.app ? ` for ${parsed.app}` : ""}`;
    if (toolName === "computer_focus_window")
        return `Focus window ${parsed.window_id || parsed.title || parsed.app || ""}`.trim();
    if (toolName === "computer_wait")
        return "Wait for desktop state";
    if (toolName === "computer_get")
        return `Read ${parsed.property || "text"} from ${parsed.ref || "element"}`;
    if (toolName === "bash")
...
function formatEntriesForRecap(entries, maxChars) {
    const lines = [];
...
function formatRecapEntry(entry) {
...
function isAuthenticationError(error) {
    const message = error instanceof Error ? error.message : String(error);
    return /\b(401|403)\b|unauthori[sz]ed|invalid.*(api[_ ]?key|token|credential)|authentication failed|forbidden|access denied/i.test(message);
}
...
function extractResponseDetail(body) {
    if (!body)
        return null;
    try {
