...
function truncateForSummary(text, maxChars = TOOL_RESULT_MAX_CHARS) {
...
export function createCompactionSummaryMessage(summary) {
    return {
        role: "system",
...
export function getCompactionSummaryText(message) {
...
export function estimateMessageTokens(message) {
    ...
    switch (message.role) {
        ...
        case "system":
            ...
            break;
        default:
            chars += stringifyForSummary(message.content).length;
            break;
    }
    return Math.ceil(chars / 4);
}
export function estimateConversationTokens(systemPrompt, messages, inFlightText = "") {
...
function findTurnStartIndex(messages, entryIndex, startIndex) {
...
export function findCutPoint(messages, startIndex, keepRecentTokens) {
    const cutPoints = [];
...
export function prepareCompaction(messages, systemPrompt, settings) {
...
export function relaxCompactionSettings(settings) {
    return {
        ...settings,
...
export function serializeConversation(messages) {
    ...
    for (const message of messages) {
        ...
        if (message.role === "system") {
            const content = typeof message.content === "string" ? message.content.trim() : getTextParts(message.content).join("\n").trim();
            if (content)
...
async function summarizeConversation(provider, modelId, messages, reserveTokens, customInstructions, previousSummary, promptOverride, signal) {
...
export async function generateCompactionSummary(provider, modelId, preparation, customInstructions, signal) {
    ...
    if (isSplitTurn && turnPrefixMessages.length > 0) {
        const [historySummary, prefixSummary] = await Promise.all([
            messagesToSummarize.length > 0
                ? summarizeConversation(provider, modelId, messagesToSummarize, settings.reserveTokens, customInstructions, previousSummary, undefined, signal)
                : Promise.resolve(previousSummary?.trim() || ""),
            summarizeConversation(provider, modelId, turnPrefixMessages, settings.reserveTokens, undefined, undefined, TURN_PREFIX_SUMMARIZATION_PROMPT, signal),
        ]);
    ...
    return summarizeConversation(provider, modelId, messagesToSummarize, settings.reserveTokens, customInstructions, previousSummary, undefined, signal);
}
