...
async function importAgentModuleWithBatchMocks() {
    ...
    vi.doMock("../grok/batch", async () => {
        const actual = await vi.importActual("../grok/batch");
        return {
            ...actual,
            createBatch,
            addBatchRequests,
...
describe("Agent batch mode", () => {
    it("throws when a child batch response has no choices", async () => {
        ...
        const runTaskRequestBatch = agent.runTaskRequestBatch.bind(agent);
        await expect(runTaskRequestBatch({
            request: {
                agent: "general",
...
//# sourceMappingURL=batch-mode.test.js.map