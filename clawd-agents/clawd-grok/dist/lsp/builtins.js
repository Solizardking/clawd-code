...
const BUILT_IN_DEFINITIONS = {
    typescript: {
        ...
        async resolveLaunch(root, settings) {
            ...
            const resolved = override?.command && override?.args
                ? { command: override.command, args: override.args }
                : await resolveNodeServerLaunch(root, "typescript-language-server", "typescript-language-server", ["--stdio"], settings.autoInstall);
            if (!resolved && !override?.command)
                return null;
        ...
        async resolveLaunch(root, settings) {
            ...
            const resolved = override?.command && override?.args
                ...
                : (await resolveNodeServerLaunch(root, "pyright-langserver", "pyright", ["--stdio"], settings.autoInstall)) ||
                    (await resolveNodeServerLaunch(root, "basedpyright-langserver", "basedpyright", ["--stdio"], settings.autoInstall));
            if (!resolved && !override?.command)
                return null;
            return {
                command: override?.command ?? resolved?.command ?? "",
    ...
    "bash-language-server": {
        ...
        async resolveLaunch(root, settings) {
            ...
            const resolved = override?.command && override?.args
                ? { command: override.command, args: override.args }
                : await resolveNodeServerLaunch(root, "bash-language-server", "bash-language-server", ["start"], settings.autoInstall);
            if (!resolved && !override?.command)
                return null;
            return {
                command: override?.command ?? resolved?.command ?? "",
    ...
    "yaml-language-server": {
        ...
        async resolveLaunch(root, settings) {
            ...
            const resolved = override?.command && override?.args
                ? { command: override.command, args: override.args }
                : await resolveNodeServerLaunch(root, "yaml-language-server", "yaml-language-server", ["--stdio"], settings.autoInstall);
            if (!resolved && !override?.command)
                return null;
            return {
                command: override?.command ?? resolved?.command ?? "",
...
export function createRuntimeLspDefinitions(cwd, settings) {
    const definitions = [];
...
function createCustomRuntimeDefinition(cwd, config) {
    return {
        id: config.id,
        extensions: normalizeExtensions(config.extensions),
        languageIds: config.languageIds ?? Object.fromEntries(config.extensions.map((extension) => [extension, extension.slice(1)])),
...
async function resolveCommand(root, binary) {
    const localBinary = await findLocalBinary(root, binary);
    if (localBinary)
        return localBinary;
    return findCommandOnPath(binary);
}
async function resolveTypeScriptServer(root) {
...
async function findLocalBinary(root, binary) {
    const ext = process.platform === "win32" ? ".cmd" : "";
    const candidate = path.join(root, "node_modules", ".bin", `${binary}${ext}`);
    return (await pathExists(candidate)) ? candidate : null;
}
async function resolveNodeServerLaunch(root, binary, packageName, baseArgs, autoInstall) {
...
async function findCommandOnPath(binary) {
...
async function findNearestRoot(filePath, cwd, markers) {
