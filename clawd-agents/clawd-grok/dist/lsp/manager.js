...
import { createRuntimeLspDefinitions } from "./builtins";
import { createLspClientSession } from "./client";
export function createWorkspaceLspManager(cwd, settings, options = {}) {
    ...
    async function getClientsForFile(filePath) {
        ...
        if (matches.length === 0)
            return [];
        const resolved = await Promise.all(matches.map(async (definition) => {
    ...
    async function syncFile(filePath, content, save = true, waitForDiagnostics = true) {
        ...
        const diagnostics = await Promise.all(records.map(async ({ key, definition, client }) => {
        ...
        return diagnostics.filter((entry) => entry !== null && entry.diagnostics.length > 0);
    }
    async function query(input) {
        ...
        const results = (await Promise.all(records.map(async ({ key, client }) => {
            if (input.operation === "incomingCalls" || input.operation === "outgoingCalls") {
                try {
                    const items = await client.sendRequest("textDocument/prepareCallHierarchy", params);
                    const firstItem = Array.isArray(items) ? items[0] : undefined;
                    if (!firstItem)
                        return [];
                    return client.sendRequest(input.operation === "incomingCalls" ? "callHierarchy/incomingCalls" : "callHierarchy/outgoingCalls", { item: firstItem });
                }
                catch {
                    clients.delete(key);
                    return [];
            ...
            try {
                return await client.sendRequest(getOperationMethod(input.operation), params);
            }
            catch {
                clients.delete(key);
                return [];
