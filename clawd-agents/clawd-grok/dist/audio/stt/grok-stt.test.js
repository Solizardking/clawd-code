...
describe("GrokSttEngine", () => {
    ...
    it("omits language and format fields when language is not provided", async () => {
        ...
        const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ text: "ok", duration: 0.5 }), {
            status: 200,
    ...
    it("surfaces non-2xx responses with status and body", async () => {
        ...
        global.fetch = vi
            .fn()
            .mockResolvedValue(new Response("invalid audio", { status: 400, statusText: "Bad Request" }));
        const engine = new GrokSttEngine({ apiKey: "test-key" });
    ...
    it("rejects empty transcripts", async () => {
        const audioPath = path.join(tempDir, "clip.mp3");
        fs.writeFileSync(audioPath, "mp3-bytes");
        global.fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ text: "   ", duration: 0.1 }), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        }));
        const engine = new GrokSttEngine({ apiKey: "test-key" });
...
//# sourceMappingURL=grok-stt.test.js.map