...
function hasExplicitSandboxOption(options) {
    return options.sandbox === true || options.sandbox === false;
}
async function promptWorkspaceTrust(cwd, sandboxSupported = isShuruSandboxSupported()) {
...
async function resolveWorkspaceTrustSandboxMode(sandboxMode, options) {
    if (sandboxMode === "shuru" || hasExplicitSandboxOption(options))
        return sandboxMode;
    if (process.env.CLAWD_TRUST_WORKSPACE)
        return sandboxMode;
    ...
    if (decision.remember)
        saveWorkspaceTrustDecision(cwd, decision.sandboxMode);
...
program
    ...
    if (options.rpcUrl)
        process.env.SOLANA_RPC_URL = options.rpcUrl;
    if (options.apiUrl)
        process.env.PHOENIX_API_URL = options.apiUrl;
...
strategyCommand
    ...
    if (options.cancelOrders)
        console.log("  Will cancel open orders");
    if (options.closePosition)
        console.log("  Will close position");
    console.log();
});
// ===== AGENT SKILLS & MCP =====
program
