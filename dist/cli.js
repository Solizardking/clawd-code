...
import { DEFAULT_FAST_MODEL, DEFAULT_MODEL, DEFAULT_PROVIDER, DEFAULT_RESEARCH_MODEL, DEFAULT_TRADE_VISION_MODEL, DEFAULT_VISION_MODEL, defaultModelFor, getModel, isStreamingSupported, listModelsByProvider, normalizeModelId, printModelsTable, } from './grok-models.js';
import { getOpenRouterProviderModels, OPENROUTER_AUTO_MODEL, OPENROUTER_FABLE5, OPENROUTER_FABLE_LATESY, OPENROUTER_NEMO_MODEL1, OPENROUTER_NEMO_MODEL2, OPENROUTER_NEMO_MODEL3, } from './openrouter.js';
...
function normalizeProvider(provider) {
    ...
    if (normalized === 'claude' || normalized === 'ant')
        return 'anthropic';
    if (normalized === 'z' || normalized === 'zai' || normalized === 'z.ai' || normalized === 'glm')
        return 'zai';
    if (normalized === 'anthropic' || normalized === 'deepseek' || normalized === 'openrouter' || normalized === 'xai' || normalized === 'zai')
        return normalized;
...
async function cmdInspect() {
    ...
    const zai = getZaiEnvConfig(env);
    const fit = (value, width) => value.length > width ? `${value.slice(0, width - 3)}...` : value.padEnd(width);
    console.log('\n╔══════════════════════════════════════════════════════════════════════╗');
    ...
    for (const prov of ['zai', 'xai', 'anthropic', 'deepseek', 'openrouter']) {
...
async function main() {
    ...
    if (args.includes('--thinking'))
        config.zaiThinking = 'enabled';
    if (args.includes('--no-thinking'))
        config.zaiThinking = 'disabled';
