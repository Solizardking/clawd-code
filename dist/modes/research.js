    ...
    async runStreaming(query, provider, agentCount, model) {
        ...
        try {
            ...
            if (provider === 'openrouter') {
                ...
                console.log(`[RESEARCH MODE] OpenRouter ${selection.explicit ? 'model' : 'route'}: ${selection.model}` +
            ...
            if (provider === 'zai') {
                ...
                console.log(`[RESEARCH MODE] Streaming Z.AI/${useModel}` +
    ...
    async runBlocking(query, provider, agentCount, model) {
        try {
            ...
            if (provider === 'openrouter') {
                ...
                console.log(`[RESEARCH MODE] Running OpenRouter/${selection.model}` +
                    `${selection.explicit ? '' : ` (${selection.route}: ${selection.reason})`}...`);
            ...
            if (provider === 'zai') {
                const client = createZaiClient(this.config.zaiApiKey, this.config.zaiBaseUrl);
                if (!client)
                    return { content: 'ZAI_API_KEY not set.', citations: [] };
                const useModel = model.startsWith('glm-') ? model : ZAI_DEFAULT_MODEL;
                console.log(`[RESEARCH MODE] Running Z.AI/${useModel}` +
                    ` (thinking: ${this.config.zaiThinking ?? 'enabled'}, effort: ${this.config.zaiReasoningEffort ?? 'max'})...`);
                const response = await client.chat({
                    model: useModel,
                    messages: [
