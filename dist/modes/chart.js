...
import { createZaiAgentClient, createZaiClient, extractZaiSlideAgentText, extractZaiSlideAgentUrls, ZAI_AGENT_BASE_URL, ZAI_DEFAULT_MODEL, ZAI_SLIDE_AGENT_ID, ZAI_TRADE_VISION_MODEL, } from '../zai.js';
export class ChartMode {
    ...
    async run(args) {
        ...
        if (options.kind !== 'analysis')
            console.log(`[CHART MODE] Agent:   ${ZAI_SLIDE_AGENT_ID}`);
        let chartFindings = '';
    ...
    parseArgs(args) {
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (valueFlags.has(arg)) {
                if (arg === '--chart' || arg === '--image' || arg === '--screenshot')
                    imageInput = next;
                if (arg === '--conversation' || arg === '--conversation-id')
                    conversationId = next;
                if (arg === '--pages')
                    pageCount = this.parsePositiveInt(next);
                if (arg === '--model')
                    model = next;
                if (arg === '--vision-model')
                    visionModel = next;
        ...
        if (kind === 'analysis') {
            if (lowerPrompt.includes('poster'))
                kind = 'poster';
            if (lowerPrompt.includes('slide') || lowerPrompt.includes('deck') || lowerPrompt.includes('ppt'))
                kind = 'slides';
        }
    ...
    async analyzeChartImage(options, visionModel) {
    ...
    async generateChartPlan(options, model, chartFindings) {
    ...
    async generateSlidePoster(options, model, chartFindings) {
        ...
        console.log('╚════════════════════════════════════════════════════╝');
        if (created.id)
            console.log(`Request ID: ${created.id}`);
        if (conversationId)
        ...
        if (!options.exportArtifact || !conversationId)
            return;
        await this.printSlidePosterExport(agent, { ...options, conversationId });
    }
    async printSlidePosterExport(agent, options) {
    ...
    composePlannerPrompt(options, chartFindings) {
        return [
    ...
    composeSlidePosterPrompt(options, model, chartFindings) {
    ...
    printOfflinePlan(options, chartFindings) {
        console.log('\n╔════════════════════════════════════════════════════╗');
        ...
        if (options.prompt)
            console.log(`Request: ${options.prompt}`);
        if (chartFindings)
            console.log(`\nVision findings:\n${chartFindings}`);
        console.log('\nDefault workflow:');
